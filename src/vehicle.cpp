...
vector<vector<double>> Vehicle::get_target_for_state(string state, map<int, vector<vector<double>>> predictions, double duration, bool car_just_ahead) {
  // Returns two lists s_target and d_target in a single vector
  //  s_target : [s, s_dot, s_ddot]
  //  d_target : [d, d_dot, d_ddot]
  //
  // If no leading car found target lane, ego car will make up PERCENT_V_DIFF_TO_MAKE_UP of the difference between current velocity and target velocity.
  // If leading car is found, set target s to FOLLOW_DISTANCE and target s_dot to leading car's s_dot based on predictions
  ...
  // emergency brake, in the case sudden somebody braking in front of the car
  if (car_just_ahead) {
    target_s_d = 0.0;
  }
...
vector<vector<double>> Vehicle::generate_predictions(double traj_start_time, double duration) {
  ...
  // generate a list of predicted s and d positions for dummy constant-speed vehicles
  // next s : simple linear prediction
  // next d : 0 based on the other cars are perfect driver who does not do the change lane
